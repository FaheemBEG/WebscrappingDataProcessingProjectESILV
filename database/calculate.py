...
def calculate_consumption(game_title: str, platform: str):
    ...
    default_pc_co2 = default_pc["gwp_total"].values[
        0
    ]  # Source : IdeaCentre T540 Gaming (boavizta)
    ...
    xbox_one_co2 = boavizta[boavizta["name"] == "Xbox One S"]["gwp_total"].values[0] * (
        1 - boavizta[boavizta["name"] == "Xbox One S"]["gwp_use_ratio"].values[0]
    )  # Extracting carbon footprint without usage percentage
    xbox_series_x_co2 = boavizta[boavizta["name"] == "Xbox Series X"][
        "gwp_total"
    ].values[0] * (
        1 - boavizta[boavizta["name"] == "Xbox Series X"]["gwp_use_ratio"].values[0]
    )  # Extracting carbon footprint without usage percentage
    ...
    if (
        not games_data["Main Story (Hours)"].empty
        and games_data["Main Story (Hours)"].values[0] != -1
    ):
        try:
    ...
    if (
        games_data["Platforms"].values[0] != ""
        and not platform.lower() in games_data["Platforms"].values[0].lower()
    ):
    ...
    if "playstation 4" in platform.lower():
        ...
        plateform_co2 = (
            box_co2 + gpu_co2 + processor_co2 + ssd_co2 + ram_co2 + motherboard_co2
        )
...
def add_consumption(games_file_path: str = path + "/games_extrait.csv"):
    ...
    if "carbon_footprint_pc" not in df_games.columns:
        ...
        df_games["carbon_footprint_pc"] = df_games.apply(
            lambda row: calculate_consumption(row["Title"], "PC")[
    ...
    for console in consoles:
        ...
        def calculate_consumption_console(row):
            if pd.notna(row["Platforms"]):  # Vérifier si la valeur n'est pas NaN
                ...
                if console.lower() in plateform:
                    return calculate_consumption(row["Title"], plateform=console)[
                        "Carbon_footprint_kgco2"
                    ]
        ...
        df_games[
            f"carbon_footprint_{console.lower().replace(' ', '_')}"
        ] = df_games.apply(calculate_consumption_console, axis=1)
